<!-- #done -->
<div class="alert alert-top alert-dismissible fade show m-0 w-100 d-flex gap-2 align-items-center has-edit-buttons" role="alert" data-scroll="true">
	<!-- add data-scroll="false" to stop scroll -->
	<div class="center-center">
		<div class="rz-btns-container">
			<span class="rzBtn rzBtn-xs">Edit</span>
		</div><!-- /.rz-btns-container -->
	</div><!-- /.center-center -->

	<div class="marquee-wrap overflow-hidden w-100 d-flex">
		<div class="marquee d-flex gap-2 align-items-center">
			<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path fill-rule="evenodd" clip-rule="evenodd" d="M12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20ZM12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" fill="black"/>
				<path fill-rule="evenodd" clip-rule="evenodd" d="M11 17.0009V11H13V17.0009H11Z" fill="black"/>
				<path fill-rule="evenodd" clip-rule="evenodd" d="M11 9.00089V7H13V9.00089H11Z" fill="black"/>
			</svg>
			<p class="m-0"><strong>Holy guacamole!</strong> You should check in on some of those Vestibulum id ligula porta felis euismod semper. Duis mollis, est non commodo luctus <a href="#section">below.</a></p>
		</div><!--/.marquee -->
	</div><!-- /.marquee-wrap -->
	<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
</div><!-- /.alert -->